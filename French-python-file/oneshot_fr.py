...
OUTPUT_DIR = "Benchmark_Results/French_OneShot_Final"
NUM_SHOTS = 1  # <--- Using strictly 1 Example

# ==========================================
# 2. DATA PROCESSING
# ==========================================
def load_and_clean_csv(filepath):
    """Loads CSV and strips whitespace from headers."""
    if not os.path.exists(filepath):
        print(f"[ERROR] File not found: {filepath}")
        sys.exit(1)
    df = pd.read_csv(filepath)
    df.columns = df.columns.str.strip() # Fixes " NER" space bug
    return df
...
def merge_gold_data(row):
    """
    Union Strategy: PII + NER = Master Truth
    """
    # 1. PII Column
    pii_dict = parse_gold_labels(row.get('french_pii_detection', ''))
    
    # 2. NER Column (Cleaned name)
...
# 3. PROMPT & INFERENCE
# ==========================================
def get_oneshot_prompt(tokenizer, test_text, example_row):
    """
    Builds the 1-Shot history.
    """
    system_msg = (
    ...
    ex_label = merge_gold_data(example_row)
...
def run_inference():
    ...
    # Pick 1 Random Example (Seed 42 for consistency)
    # Using df_train.iloc[0] is okay if shuffled, but sample is safer
