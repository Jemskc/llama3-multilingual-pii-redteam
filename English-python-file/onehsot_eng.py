...
TRAIN_FILE = "english_train.csv"  # The Source for the Example (400 rows)
OUTPUT_DIR = "Benchmark_Results/English_OneShot"
...
def setup_environment():
    ...
    if not os.path.exists(TRAIN_FILE):
        print(f"[CRITICAL ERROR] {TRAIN_FILE} not found. Cannot run One-Shot without training examples.")
        sys.exit(1)
...
def parse_gold_labels(label_str):
    """Parses Custom Tags like 'PHONE_NUMBER(123)' into dictionary."""
...
def get_oneshot_prompt(tokenizer, test_text, train_row):
    """
    Constructs a prompt with ONE example from the training set.
    """
    # 1. Prepare the Example
    ex_text = str(train_row['text'])
    ex_label_dict = parse_gold_labels(train_row['detected_entities'])
    # Format dictionary as strict JSON string
    ex_response = str(ex_label_dict).replace("'", '"')
    ...
    # 2. Build Messages: System -> Example User -> Example Assistant -> Real User
    messages = [
        {"role": "system", "content": system_msg},
        # The One Shot (History)
        {"role": "user", "content": f"Text: {ex_text}\nOutput JSON:"},
    ...
    try:
        ...
        prompt = (f"System: {system_msg}\n"
                  f"User: Text: {ex_text}\nOutput JSON:\n"
                  f"Assistant: {ex_response}\n"
                  f"User: Text: {test_text}\nOutput JSON:\nAssistant:")
    return prompt
...
def run_inference():
    print("-" * 60)
    print(f"[START] One-Shot Benchmark")
    print("-" * 60)
    
    df_test = pd.read_csv(TEST_FILE)
    df_train = pd.read_csv(TRAIN_FILE)
    
    # Pick ONE random example to use for all prompts 
    # Using random_state=42 ensures the SAME example is used every time you run this script
    example_row = df_train.sample(n=1, random_state=42).iloc[0]
    print(f"[INFO] One-Shot Example Selected ID: {example_row.get('id', 'Unknown')}")
    ...
    for i, model_id in enumerate(MODELS_TO_TEST):
        ...
        for idx, row in pbar:
            ...
            # Generate One-Shot Prompt using the Helper
            prompt = get_oneshot_prompt(tokenizer, text, example_row)
