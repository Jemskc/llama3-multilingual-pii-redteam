...
TEST_FILE = "spanish_test.csv"
TRAIN_FILE = "spanish_train.csv"
OUTPUT_DIR = "Benchmark_Results/Spanish_OneShot_Final"
NUM_SHOTS = 1  # <--- Using 1 Example

# ==========================================
# 2. DATA PROCESSING (Robust Logic)
# ==========================================
def load_and_clean_csv(filepath):
    """Loads CSV and strips whitespace/quotes from headers."""
    if not os.path.exists(filepath):
        print(f"[ERROR] File not found: {filepath}")
        sys.exit(1)
    df = pd.read_csv(filepath)
    # Critical: Strip spaces and remove potential quote characters from headers
    df.columns = df.columns.str.strip().str.replace('"', '')
    return df

def find_ner_column(df):
    """Dynamically finds the column containing 'NER' and 'spanish'."""
...
def merge_gold_data(row, ner_col_name):
    """Union Strategy: spanish_pii_detection + NER column"""
    # 1. Parse PII Column
...
# 3. PROMPT GENERATOR
# ==========================================
def get_oneshot_prompt(tokenizer, test_text, example_row, ner_col_name):
    """
    Constructs a 1-Shot prompt history.
    """
    system_msg = (
    ...
    # 1. The One Shot Example
    ex_text = str(example_row['text'])
    # Merge labels for the example so the model learns the full scope
    ex_label = merge_gold_data(example_row, ner_col_name)
...
def run_inference():
    ...
    # Find NER Column
    ner_col_name = find_ner_column(df_test)
    
    # Pick 1 Random Example (Seed 42)
    example_row = df_train.sample(n=1, random_state=42).iloc[0]
    print(f"[INFO] Using One-Shot Example ID: {example_row.get('id', 'Unknown')}")
    ...
    for model_id in MODELS_TO_TEST:
        ...
        for idx, row in tqdm(df_test.iterrows(), total=len(df_test), desc=f"   > Testing"):
            ...
            # Merge Gold Labels
            gold_label = merge_gold_data(row, ner_col_name)
            
            # Generate Prompt using Example and NER Column Name
            prompt = get_oneshot_prompt(tokenizer, text, example_row, ner_col_name)
