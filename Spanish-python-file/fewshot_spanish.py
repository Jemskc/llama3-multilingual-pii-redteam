...
OUTPUT_DIR = "Benchmark_Results/Spanish_FewShot_Final"
NUM_SHOTS = 5  # <--- Using 5 Examples
...
# 3. PROMPT GENERATOR
# ==========================================
def get_fewshot_prompt(tokenizer, test_text, example_rows, ner_col_name):
    """
    Constructs a 5-Shot prompt history.
    """
    system_msg = (
    ...
    # Loop through 5 Examples
    for _, row in example_rows.iterrows():
        ex_text = str(row['text'])
        ex_label = merge_gold_data(row, ner_col_name)
    ...
    try:
        ...
        for _, row in example_rows.iterrows():
            ex_l = str(merge_gold_data(row, ner_col_name)).replace("'", '"')
            prompt += f"User: Text: {row['text']}\nOutput JSON:\nAssistant: {ex_l}\n"
...
def run_inference():
    ...
    for model_id in MODELS_TO_TEST:
        ...
        for idx, row in tqdm(df_test.iterrows(), total=len(df_test), desc=f"   > Testing"):
            ...
            prompt = get_fewshot_prompt(tokenizer, text, example_rows, ner_col_name)
            inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
            
            with torch.no_grad():
                # Increased token limit for 5-shot response context
