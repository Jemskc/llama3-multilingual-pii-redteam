...
OUTPUT_DIR = "Benchmark_Results/Spanish_ZeroShot_Final"
...
def find_ner_column(df):
    """
    Dynamically finds the column containing 'NER' and 'spanish' (case insensitive).
    This handles variations like ' NER FOUND...' or 'NER Found...'.
    """
    for col in df.columns:
        if "NER" in col and "spanish" in col.lower():
            print(f"[INFO] Found NER Column: '{col}'")
            return col
    print("[WARNING] Could not find a specific NER column. Only PII column will be used.")
    return None
...
def parse_gold_labels(label_str):
    ...
    # Standardize tags (Spanish dataset likely uses similar tags to French/English)
    tag_map = {
...
def run_inference():
    ...
    for model_id in MODELS_TO_TEST:
        ...
        for idx, row in tqdm(df_test.iterrows(), total=len(df_test), desc=f"   > Testing"):
            ...
            for key in all_keys:
                g = set([x.lower().strip() for x in gold_label.get(key, [])])
                p = set([str(x).lower().strip() for x in pred_label.get(key, [])])
